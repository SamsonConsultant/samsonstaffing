...
@section('content')
    ...
    @include('pages.employer.navigation', ['title' => 'Contact Add / Edit'])
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    ...
                    <a style="margin-top:20px;" class="btn btn-default" href="{{ route('employer.contacts.index') }}">
                        Back to List
                    </a>
                </div>

                <div class="card-body">
                    <form action="{{ route("employer.contacts.store") }}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @csrf
                        <input type="hidden" name="redirect_url" id="redirect_url" value="{{ route('employer.contacts.index') }}">
                        @if(isset($contact))
                            ...
                            <div class="col-md-6 form-group {{ $errors->has('phone_code') ? 'has-error' : '' }}">
                                ...
                                <select name="phone_code" class="form-control select2">
                                    ...
                                    @foreach($country as $ct)
                                        <option value="{{ $ct->phonecode }}" @if(isset($contact) && $ct->phonecode == $contact->phone_code) selected @endif>{{ $ct->name }} ({{ $ct->phonecode }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 form-group {{ $errors->has('account_id') ? 'has-error' : '' }}">
                                ...
                                <select name="account_id" class="form-control" required>
                                    ...
                                    @forelse($companies as $com)
                                        <option value="{{ $com->id }}" @if(isset($contact) && $com->id == $contact->account_id) selected @endif>{{ $com->title }}</option>
                                    @empty
                                    @endforelse
                                </select>
