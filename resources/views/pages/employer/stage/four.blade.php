                    ...
                    @forelse($jobs as $post)
                        <tr data-entry-id="{{ $post->id }}">
                            ...
                            <td>
                                ...
                                @if($post->selection_status == '1')
                                    <a href="{{ route('admin.job.status', [$post->id, '5', Auth::user()->id]) }}" class="btn btn-info btn-sm" onclick="return confirm('Are you sure.?')">Joining in Progress</a>
                                @else
                                    <button type="button" class="btn btn-warning btn-sm inter-btn" data-id="{{ $post->id }}" data-user="{{ $post->user->name }}" data-toggle="modal" data-target="#selectionModal">Selection</button>
                                @endif
                            </td>
                        </tr>
                    @empty
...
@include('pages.common.selection')