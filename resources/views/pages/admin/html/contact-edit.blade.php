<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.contacts.update", [$contact->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6 form-group {{ $errors->has('phone_code') ? 'has-error' : '' }}">
                    ...
                    <select name="phone_code" class="form-control select2">
                        ...
                        @foreach($country as $ct)
                            <option value="{{ $ct->phonecode }}" @if($ct->phonecode == $contact->phone_code) selected @endif>{{ $ct->name }} ({{ $ct->phonecode }})</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 form-group {{ $errors->has('account_id') ? 'has-error' : '' }}">
                    ...
                    <select name="account_id" class="form-control" required>
                        ...
                        @forelse($companies as $com)
                            <option value="{{ $com->id }}" @if($com->id == $contact->account_id) selected @endif>{{ $com->title }}</option>
                        @empty
                        @endforelse
                    </select>
