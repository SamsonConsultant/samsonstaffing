<div class="card">
    <div class="card-header">
        ...
        <a style="margin-top:20px;" class="btn btn-default" href="{{ route('admin.companies.index') }}">
            Back to List
        </a>
    </div>

    <div class="card-body">
        <form action="{{ route("admin.companies.update", [$company->id]) }}" method="POST" enctype="multipart/form-data" id="admin-form">
            @csrf
            @method('PUT')
            <input type="hidden" name="redirect_url" id="redirect_url" value="{{ route('admin.companies.index') }}">            
            <div class="row">
                ...
                {{-- <div class="form-group {{ $errors->has('account_number') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="account_number" name="account_number" class="form-control" value="{{ old('account_number', isset($company) ? $company->account_number : '') }}" required>
                ...
                <div class="col-md-6 form-group {{ $errors->has('account_type') ? 'has-error' : '' }}">
                    ...
                    {{-- <input type="text" id="account_type" name="account_type" class="form-control" value="{{ old('account_type', isset($company) ? $company->account_type : '') }}" required> --}}
                    <select name="account_type" class="form-control">
                        <option value="">-- Select --</option>
                        @forelse($posts as $post)
                            <option value="{{ $post['post_title'] }}" @if($company->account_type == $post['post_title']) selected @endif>{{ $post['post_title'] }}</option>
                        @empty
                        @endforelse
                    </select>
                ...
                <div class="col-md-6 form-group {{ $errors->has('country_id') ? 'has-error' : '' }}">
                    ...
                    <select name="country_id" class="form-control select2" id="country-list">
                        ...
                        @foreach($country as $ct)
                            <option value="{{ $ct->id }}" @if($ct->id == $company->country_id) selected @endif>{{ $ct->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 form-group {{ $errors->has('state_id') ? 'has-error' : '' }} state-list">
                    ...
                    <select name="state_id" class="form-control select2">
                        <option value="">-- Select --</option>
                        @foreach($state as $st)
                            <option value="{{ $st->id }}" @if($st->id == $company->state_id) selected @endif>{{ $st->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 form-group {{ $errors->has('phone_code') ? 'has-error' : '' }}">
                    ...
                    <select name="phone_code" class="form-control select2">
                        <option value="">-- Select --</option>
                        @foreach($country as $ct)
                            <option value="{{ $ct->phonecode }}" @if($ct->phonecode == $company->phone_code) selected @endif>{{ $ct->name }} ({{ $ct->phonecode }})</option>
                        @endforeach
                    </select>
